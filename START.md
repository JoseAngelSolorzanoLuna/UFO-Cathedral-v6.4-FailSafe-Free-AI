# UFO Cathedral v6.7 — start here

Live pack: https://ai4pcfree.netlify.app
Builder: José Angel Solorzano Luna

v6.7 companion is software-first on the same Gate + Ledger.
Body kits and neuro-chip are not software-ready.

---

# UFO-Cathedral v6.6 Pristine — Fail-Safe Free AI
**Version: v6.6 Pristine (unified — was v6.4/v6.5/v7.0 FLAWLESS mixed, now single badge)**

**Honest Story — How it was really built (not one model lacking reasoning):**
This was NOT done by downloading a single AI model that lacks reasoning.
Process was: first download Llama, then the bots from there on out, then all 5 together.
- 5 Geometric Models as Opcode Families: Phi3 sphere, Llama pyramid, Llama graph, Qwen torus, Llava eye — distinct opcode spaces, not one dense model
- High-Throughput Cathedral Computer: A-T-V-Q-P Pipeline + 5 Model Register File — throughput on 8GB optimized
- Shadow Mode: 4 Brains + 1 Eyes + 5 Models → Coherence Evaluation → SAFE | HOLD | ESCALATE
- Bounded Sensor Gate: Merkle Permit + Rate Limit + Click-Fence — prevents loops, malware download, crashes

**Honest RAM Path (fixes 8GB five models + vision claim):**
- **Lite (<8GB phones):** `ollama pull phi3:mini` — 1.8GB Q4 — COD HUD taps only — no Council, single reasoning (like basic Discord bot)
- **Standard (iPhone 16 Pro Max 8GB — your device):** `phi3:mini` + `llama3.2:3b` — 3.2GB total — learns sniper, Mobilerun red boxes, OBS live — 2-model reasoning
- **Full Cathedral (PC 8GB only):** All 5 opcode families together — Phi3 + Llama pyramid + Llama graph + Qwen + Llava — 5.8-6.2GB — full Council + Ledger Hash `H(n)=SHA256(H(n-1)||state||ac[...]

We do NOT claim five models + vision on iPhone. START.md now says pull phi3:mini first as entry, then add Llama and rest to get full reasoning.

**Quick Start:**

```bash
# 1. Lite entry — honest first step
ollama pull phi3:mini

# 2. Standard — your 16 Pro Max test
pip install droidrun
brew install scrcpy
python ufo64.py --mode phone --task "Tap sniper button only inside COD HUD"

# 3. COD HUD Learn (giftable sniper)
python ufo64.py --mode cod_hud
# exports my_sniper_v1.jsonl for OBS + TikTok Live

# 4. Full PC Cathedral (all 5 together for reasoning)
ollama pull llama3.2:3b
ollama pull qwen2:1.5b
ollama pull llava:phi3
python ufo64.py --mode pc
python verify_ledger.py
```

**Safety Card — Now Saved State (not just README spec):**
- File: `~/.ufo/ledger/safety_state.json`
- `UNVERIFIED -> 3 replays -> TRUTH`
- `verify_ledger.py` checks both ledger hash chain AND safety_state.json

**Version Unification:**
- Badge everywhere: `v6.6 Pristine — 8GB / iPhone 16 Pro Max Compatible`
- Delete `v7.0 FLAWLESS` until phone driver + safety state + 5-model proof passes
- HTML demo (PC->Phone/COD HUD) now lives in `ufo64.py` too, not just HTML pack

**OmniGood Guardrails:**
- No spam/flooding (rate + embedding drift, only last 8 messages, no crash)
- No bashing/heated arguments — formal civilized
- No racism/degrading — auto ask to stay respectful
- +18 fictional euphoric allowed but not rape/non-consensual/wax without permission — therapist analogy, communication is key
- Dangerous admin step warning + alternative + email to SolorzanoLunaJose@gmail.com with obfuscated backup (1↔9,2↔8,3↔7,4↔6,5↔5) — only robot decodes, e.g., 123456789→987654321
